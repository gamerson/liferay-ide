    ...
    implements IWebsphereServer, IWebsphereServerWorkingCopy, IServerListener
{
    ...
    public boolean canMakeAdminConnection()
    {
        String host = getServer().getHost();
        String soap = getWebsphereSOAPPort();
        ...
        new Thread()
        {
            ...
            public void run()
            {
                try
                {
                    sleep( WebspherePreferences.getAdminConnectionTimeout() );
                    socket.close();
                }
                catch( Exception e )
                {
        ...
        try
        {
            status = SocketUtil.canConnect( socket, host, soap );
        }
        catch( Exception e )
        {
        }
     ...
     * (non-Javadoc)
     * @see
     * com.liferay.ide.eclipse.server.ee.websphere.core.IWebsphereServerWorkingCopy#validate(org.eclipse.core.runtime
     * .IProgressMonitor)
     */

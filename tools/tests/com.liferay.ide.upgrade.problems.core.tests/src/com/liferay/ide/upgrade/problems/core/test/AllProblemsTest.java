...
public class AllProblemsTest {
	...
	public void allProblems() throws Exception {
		...
		List<UpgradeProblem> problems = m.findUpgradeProblems(
			new File("../com.liferay.blade.upgrade.liferay70/projects/"), versions, new NullProgressMonitor());
		...
		for (UpgradeProblem problem : problems) {
			if ((problem.getHtml() != null) && problem.getHtml().equals("#legacy")) {
				continue;
			}

			Assert.assertNotNull("problem.title=" + problem.getTitle() + ", problem.getResource()=" + problem.getResource(), problem.getHtml());
			Assert.assertTrue(
				"problem.title=" + problem.getTitle() + ", problem.getResource()=" + problem.getResource(), problem.getHtml().length() > 0);
		}
	}

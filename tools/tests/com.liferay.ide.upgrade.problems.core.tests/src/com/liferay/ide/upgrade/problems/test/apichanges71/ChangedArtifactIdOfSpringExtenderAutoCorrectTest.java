 ...
 * @author Seiphon Wang
 */
public class ChangedArtifactIdOfSpringExtenderAutoCorrectTest {

	@Test
	public void autoCorrectProblems() throws Exception {
		File tempFolder = Files.createTempDirectory("autocorrect").toFile();

		File testFile = new File(tempFolder, "build.gradle");

		tempFolder.deleteOnExit();

		File originalTestfile = new File("tests/files/build.gradle");
		...
		for (ServiceReference<FileMigrator> mref : mrefs) {
			...
			if (clazz.getName().contains("ChangedArtifactIdOfSpringExtender")) {

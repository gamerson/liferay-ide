...
public class InitBundleTaskAction extends GradleTaskAction
{
    ...
    protected void afterTask()
    {
        ...
        String name = LiferayWorkspaceUtil.getLiferayWorkspaceProjectBundlesDir( project );

        IPath bundlesLocation = project.getFile( name ).getLocation();

        try
        {
            if( bundlesLocation.toFile().exists() )
            {
                ServerUtil.addPortalRuntimeAndServer( name, bundlesLocation, new NullProgressMonitor() );
            }
        }

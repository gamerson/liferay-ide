...
public interface NewModuleOp extends ExecutableElement
{
    ...
    @Listeners( NewModuleProjectNameListener.class )
    @Required
    @Services
    ( 
        { 
    ...
    @Type( base = ILiferayOsgiModuleProvider.class )
    @Services
    (
        value=
        {
            @Service( impl = NewModuleProviderPossibleValuesService.class ),
            @Service( impl = NewModuleProviderDefaultValueService.class )
        }
    )    
    ValueProperty PROP_MODULE_PROVIDER = new ValueProperty( TYPE, "ModuleProvider" ); //$NON-NLS-1$

    Value<NewLiferayOsgiModuleProvider<NewModuleOp>> getModuleProvider();
    void setModuleProvider( String value );
    void setModuleProvider( NewLiferayOsgiModuleProvider<NewModuleOp> value );

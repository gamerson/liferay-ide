    ...
    public void perform( Object entry )
    {
        if( entry instanceof PropertiesFile )
        {
            ...
            if( !CoreUtil.isNullOrEmpty( path ) )
            {
                ...
                try
                {
                    final String launchCmd = ServerUIUtil.formShowInSytemExplorerCommand( filePath.toFile() );

                    if( !CoreUtil.isNullOrEmpty( launchCmd ) )
                    {
                        ServerUIUtil.openInSystemExplorer( launchCmd, filePath.removeLastSegments( 1 ).toFile() );
                    }
                }
                catch( IOException e )
                {
    ...
    public void selectionChanged( IStructuredSelection sel )
    {
        ...
        while( iterator.hasNext() )
        {
            ...
            if( obj instanceof PropertiesFile )
            {
                ...
                try
                {
                    if( !CoreUtil.isNullOrEmpty( ServerUIUtil.formShowInSytemExplorerCommand( new Path( path ).toFile() ) ) &&
                        accept( node ) )
                    {
                        enabled = true;
                    }

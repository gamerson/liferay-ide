...
public class PortalServerPortsEditorSection extends AbstractPortalServerEditorSection {
	...
	protected void doValidate() {
		...
		String extGogoShellPort = _getGogoShellPort();
		...
		if (!Objects.equals(gogoShellPort, extGogoShellPort)) {
			String errorMessage = "The customized gogo-shell port is not equals defined value in portal-ex.properties.";
	...
	protected void setDefault() {
		...
		String gogoPort = _getGogoShellPort();

		execute(new SetGogoShellPortCommand(server, gogoPort));

		gogoShellPort.setText(gogoPort);
	}
	...
	private String _getGogoShellPort() {
		...
		if (Objects.isNull(liferayRuntime)) {
			File[] extPropertiesFiles = _getExtPropertiesFiles(liferayRuntime);

			if (Objects.nonNull(extPropertiesFiles)) {
				Properties extProperties = PropertiesUtil.loadProperties(extPropertiesFiles[0]);

				String gogoShellConnectString = extProperties.getProperty("module.framework.properties.osgi.console");

/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */

package com.liferay.ide.upgrade.steps.core.internal.prerequisite;

/**
 * @author Gregory Amerson
 */
public class CheckForUpgradePlannerUpdatesStepKeys {

	public static final String DESCRIPTION =
		"<p>Before contining an update plan, it is recommended to update to the latest version of Liferay Developer " +
		"Studio or Liferay IDE (whichever version you are using).  Click <b>Perform</b> to check for updates.  If " +
		"there is an update available, installed it, and afterwards you can return ot this step in upgrade plan and " +
		"continue the upgrade plan.<br/><br/><b>This step has not been implemented.</b> Select <b>Click to " +
		"Complete</b> to continue.</p>";

	public static final String ID = "check_for_upgrade_planner_updates";

	public static final String TITLE = "Check For Upgrade Planner Updates";

}
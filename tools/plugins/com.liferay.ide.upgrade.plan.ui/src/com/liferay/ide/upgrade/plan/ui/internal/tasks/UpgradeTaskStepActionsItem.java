 ...
 * @author Terry Jia
 */
public class UpgradeTaskStepActionsItem implements IExpansionListener, UpgradeItem, UpgradeListener {

	public UpgradeTaskStepActionsItem(
		FormToolkit formToolkit, ScrolledForm scrolledForm, UpgradeTaskStepActions upgradeTaskStepActions) {

		_formToolkit = formToolkit;
		_scrolledForm = scrolledForm;
		_upgradeTaskStepActions = upgradeTaskStepActions;
		...
		if (_upgradeTaskStepActions == null) {
			return;
		}
		...
		ImageHyperlink completeImageHyperlink = createImageHyperlink(
			...
			"Completing " + _upgradeTaskStepActions.getTitle() + "...", this::_complete);
		...
		ImageHyperlink skipImageHyperlink = createImageHyperlink(
			...
			"Skipping " + _upgradeTaskStepActions.getTitle() + "...", this::_skip);
		...
		_updateEnablement(upgradeTaskStepActions, _enables);
	}
	...
	public void expansionStateChanged(ExpansionEvent expansionEvent) {
		ISelection selection = new StructuredSelection(_upgradeTaskStepActions);
	...
	public void onUpgradeEvent(UpgradeEvent upgradeEvent) {
		if (upgradeEvent instanceof UpgradePlanElementStatusChangedEvent) {
			...
			if (upgradePlanElement instanceof UpgradeTaskStepAction) {
				...
				if (upgradeTaskStepAction.equals(_upgradeTaskStepActions)) {
					UIUtil.async(() -> _updateEnablement(_upgradeTaskStepActions, _enables));
				}
			}
		}
	}
	...
	private IStatus _complete(IProgressMonitor progressMonitor) {
		_upgradeTaskStepActions.setStatus(UpgradePlanElementStatus.COMPLETED);

		return Status.OK_STATUS;
	}

	private void _createDescriptionArea(Composite composite) {
		String description = _upgradeTaskStepActions.getDescription();
		...
		List<UpgradeTaskStepAction> actions = _upgradeTaskStepActions.getActions();
		...
		for (int i = 0; i < s.length; i++) {
			...
			if (i != (s.length - 1)) {
				UpgradeTaskStepAction action = actions.get(i);

				Hyperlink link = _formToolkit.createHyperlink(composite, action.getTitle(), SWT.NONE);
				...
				link.addHyperlinkListener(
					new HyperlinkAdapter() {
						...
						public void linkActivated(HyperlinkEvent e) {
							new Job("Performing " + action.getTitle() + "...") {

								@Override
								protected IStatus run(IProgressMonitor progressMonitor) {
									return action.perform(progressMonitor);
								}
	...
	private IStatus _skip(IProgressMonitor progressMonitor) {
		_upgradeTaskStepActions.setStatus(UpgradePlanElementStatus.SKIPPED);
	...
	private final UpgradeTaskStepActions _upgradeTaskStepActions;

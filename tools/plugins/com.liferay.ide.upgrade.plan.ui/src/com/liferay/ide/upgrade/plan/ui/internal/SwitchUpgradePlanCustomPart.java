...
public class SwitchUpgradePlanCustomPart extends FormComponentPart implements UpgradeListener {
				...
				_tableViewer.addSelectionChangedListener(
					new ISelectionChangedListener() {
						...
						public void selectionChanged(SelectionChangedEvent event) {
							...
							UpgradePlan upgradePlan = Adapters.adapt(
								currentSelection.getFirstElement(), UpgradePlan.class);
	...
	private void _loadUpgradePlans() {
		try {
			final Job workspaceJob = new Job("Load Upgrade Plans......") {

				@Override
				public IStatus run(IProgressMonitor monitor) {
					List<UpgradePlan> loadUpgradePlans = _upgradePlanner.loadUpgradePlans();
					...
					UIUtil.async(
						new Runnable() {
							...
							public void run() {
								...
								_tableViewer.setInput(
									loadUpgradePlans.toArray(new UpgradePlan[loadUpgradePlans.size()]));

...
public class GradleUtil
{
    ...
    public static Job executeTask(final IProject project , final String task)
    {

        final Job job = new Job( project.getName() + " - " + task )
        {
            @Override
            protected IStatus run( IProgressMonitor monitor )
            {
                try
                {
                    runGradleTask( project , task , monitor );
                }
                catch( Exception e )
                {
                    return GradleCore.createErrorStatus( "Error running Gradle task " + task, e );
                }
    ...
    private static void runGradleTask( IProject project, String task, IProgressMonitor monitor ) throws CoreException
    {
        ILaunchConfiguration launchConfiguration =
                        CorePlugin.gradleLaunchConfigurationManager().getOrCreateRunConfiguration( getRunConfigurationAttributes(project,task) );

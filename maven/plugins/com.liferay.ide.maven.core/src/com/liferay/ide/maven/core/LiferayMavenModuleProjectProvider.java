...
public class LiferayMavenModuleProjectProvider
	...
	public IStatus createNewProject(NewLiferayModuleProjectOp op, IProgressMonitor monitor) throws CoreException {
		...
		String projectName = SapphireUtil.getValue(op.getProjectName());

		IPath location = PathBridge.create(SapphireUtil.getValue(op.getLocation()));

		String className = SapphireUtil.getValue(op.getComponentName());

		String liferayVersion = SapphireUtil.getValue(op.getLiferayVersion());

		String serviceName = SapphireUtil.getValue(op.getServiceName());

		String packageName = SapphireUtil.getValue(op.getPackageName());
		...
		for (PropertyKey propertyKey : propertyKeys) {
			properties.add(
				SapphireUtil.getValue(propertyKey.getName()) + "=" + SapphireUtil.getValue(propertyKey.getValue()));
		}
		...
		String projectTemplateName = SapphireUtil.getValue(op.getProjectTemplateName());

...
public class NewMavenPluginProjectProvider
	...
	public IStatus createNewProject(NewLiferayPluginProjectOp op, IProgressMonitor monitor) throws CoreException {
		...
		String groupId = SapphireUtil.getValue(op.getGroupId());
		String artifactId = SapphireUtil.getValue(op.getProjectName());
		String version = SapphireUtil.getValue(op.getArtifactVersion());
		String javaPackage = SapphireUtil.getValue(op.getGroupId());
		String activeProfilesValue = SapphireUtil.getValue(op.getActiveProfilesValue());
		IPortletFramework portletFramework = SapphireUtil.getValue(op.getPortletFramework());
		String frameworkName = NewLiferayPluginProjectOpMethods.getFrameworkName(op);

		IPath location = PathBridge.create(SapphireUtil.getValue(op.getLocation()));
		...
		String archetypeArtifactId = SapphireUtil.getValue(op.getArchetype());
			...
			if (ListUtil.isNotEmpty(archProps)) {
				for (Object prop : archProps) {
					if (prop instanceof RequiredProperty) {
						...
						PluginType pluginType = SapphireUtil.getValue(op.getPluginType());

						if (pluginType.equals(PluginType.theme)) {
							String key = rProp.getKey();

							if (key.equals("themeParent")) {
								properties.put(key, SapphireUtil.getValue(op.getThemeParent()));
							}
							else if (key.equals("themeType")) {
								properties.put(
									key, ThemeUtil.getTemplateExtension(SapphireUtil.getValue(op.getThemeFramework())));
							}
						}
						else {
		...
		if (ListUtil.isEmpty(newProjects)) {
			...
			if (pluginType.equals(PluginType.portlet)) {
				String portletName = SapphireUtil.getValue(op.getPortletName(), false);

...
public class AbstractAppChecker implements AppChecker {
	...
	public boolean installed() throws IOException {
		boolean execOutput = CommandHelper.execWithResult(_cmd).contains(_exceptResult);
		boolean execResult = CommandHelper.execWithResult(_cmd, true).contains(_exceptResult);

		if (execOutput || execResult) {
			return true;
		}

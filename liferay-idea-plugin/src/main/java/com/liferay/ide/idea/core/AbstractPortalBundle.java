...
import com.liferay.ide.idea.util.LiferayPortalValueLoader;
...
public abstract class AbstractPortalBundle implements PortalBundle {
    ...
    public AbstractPortalBundle(final Path path) {
        if (path == null) {
    ...
    public Path[] getBundleDependencyJars() {
        ...
        final Path bundleLibPath = getAppServerLibDir();
        List<File> libFiles;
    ...
    public String[] getHookSupportedProperties() {
        final Path portalDir = getAppServerPortalDir();
        final Path[] extraLibs = getBundleDependencyJars();

        return new LiferayPortalValueLoader(portalDir, extraLibs).loadHookPropertiesFromClass();
    }

    protected String getHttpPortValue(
            final File xmlFile, final String tagName, final String attriName, final String attriValue, final String targetName) {
